...
IUSE="
	...
	virtiofsd virtualbox virt-network-filter +virt-network wireshark-plugins xen zfs
"
RESTRICT="!test? ( test )"

REQUIRED_USE="
	firewalld? ( virt-network-filter )
	libvirtd? ( || ( lxc openvz qemu virtualbox xen ) )
	lxc? ( caps libvirtd )
	openvz? ( libvirtd )
	qemu? ( libvirtd )
	virt-network? ( libvirtd )
	virt-network-filter? ( virt-network )
	virtualbox? ( libvirtd )
	xen? ( libvirtd )"
...
pkg_setup() {
	...
	use virt-network-filter && CONFIG_CHECK+="
		~BRIDGE_EBT_MARK_T
		~BRIDGE_NF_EBTABLES
		~NETFILTER_ADVANCED
	...
	if use virt-network-filter ; then
